...
namespace Ryno {
	...
	void Contact::resolve(F duration)
	{
		resolve_velocity(duration);
		resolve_interpenetration(duration);
	}

	F Contact::calculate_separating_velocity() const
	{
	...
	void Contact::resolve_velocity(F duration)
	{
	...
	void Contact::resolve_interpenetration(F duration)
	{
		...
		bodies_movements[0] = offset * particles[0]->get_inverse_mass();

		if (particles[1])
			bodies_movements[1] = offset * -particles[1]->get_inverse_mass();
		else
			bodies_movements[1] = glm::vec3(0, 0, 0);

		//Each particle is moved by an offset that depends on its mass, and on the total offset
		particles[0]->add_position(bodies_movements[0]);

		if (particles[1])
			particles[1]->add_position(bodies_movements[1]);
